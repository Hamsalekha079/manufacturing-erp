  ...
  function EditableCell({ product, field, colorClass, suffix = '' }) {
    return editingCell === `${product.code}-${field}` ? (
      ...
      <span
        ...
        className={`text-sm font-medium ${colorClass} cursor-pointer hover:bg-gray-50 px-2 py-1 rounded border border-transparent hover:border-gray-300 transition`}
      >
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(product => (
                  <tr key={product.code} className="hover:bg-gray-50">
                    <td className="px-4 py-3 font-mono text-sm text-gray-500">{product.code}</td>
                        ...
                        <div className="p-4 space-y-3 bg-white">
                          ...
                          {catProducts.length > 0 && (
                            <div className="space-y-1">
                              {catProducts.map(p => (
                                <div key={p.code} className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-2 text-xs">
